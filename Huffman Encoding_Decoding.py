...
def calcFreq(data):
    ...
    for c, f in frequency.items():
        # 3mlt el freq 1st element, w el char 2nd element, 3shan a sort
        # 3la asas el frequencies
        freq.append([f, c])
...
# list feha el nodes elly h3mlha creation
nodes = []
...
for node_data in freq:
    ...
    nodes.append(Node(node_data[0], node_data[1]))
...
for node_data in sorted(freq):
    print(f'{node_data[1]} -> {node_data[0]}')
